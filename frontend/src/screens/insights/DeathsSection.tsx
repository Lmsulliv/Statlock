...
// The old Deaths screen's body: who kills you, who damages you, and when you
// die. The two ranking tables sit side by side (they're both raw, no-verdict
// counts); the timing chart keeps the full width below.
export function DeathsSection({ scope }: { scope: Scope }) {
  ...
  return (
    <>
      ...
      <QueryBoundary query={deaths}>
        {(data) => (data.games === 0 ? <DeathsEmpty /> : <DeathsBody data={data} />)}
      </QueryBoundary>
    </>
  )
}
  ...
  return (
    <>
      ...
      <div className="team-cols">
        <div>
          <h3 className="deaths-section-title">Who kills you</h3>
        ...
        <div>
          <h3 className="deaths-section-title">Who damages you</h3>
      ...
      <h3 className="deaths-section-title">When you die</h3>
      <p className="muted deaths-note">
...
// 5), mirroring the other empty states.
function DeathsEmpty() {
  const sync = useSyncStatus()
  return (
    <EmptyState title="No deaths to show yet.">
      <p>
        Which enemy heroes kill you most, who deals you the most damage, and when
        in the game you die versus the field, will appear here.
      </p>
      <AnalysisUnlockProgress feature="Death analysis" />
